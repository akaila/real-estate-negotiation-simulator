...
> **Audience:** Learners working through the M1 MCP demos in `demos/` (01–05) plus the GitHub MCP and SSE agent demos. Use this as a guided tour while you run.
> **Prerequisites:** Python environment set up; `OPENAI_API_KEY` available; for the GitHub demo, also Node.js + `GITHUB_TOKEN`.
> **Read this *while* running:** the five demo scripts in `demos/` and the two clients (`github_agent_client.py`, `sse_agent_client.py`). Each section corresponds to one demo.
...
### How to run
```bash
python demos/01_initialize_handshake.py   # no API key needed
```
...
### How to run
```bash
python demos/02_tool_loop_trace.py   # requires OPENAI_API_KEY
```
...
### How to run
```bash
python demos/03_list_all_primitives.py   # no API key needed
```
...
### How to run
```bash
python demos/04_content_types.py   # no API key needed
```
...
### How to run
```bash
python github_agent_client.py "Find popular MCP server implementations"
...
**File:** `demos/05_streamable_http_transport.py` (~70 lines)
...
python demos/05_streamable_http_transport.py --serve --port 8765

# Terminal 2: run the client
python demos/05_streamable_http_transport.py --client --port 8765
```
...
**File:** `sse_agent_client.py` (~350 lines)
...
python inventory_server.py --sse --port 8002

# Terminal 3: run the agent
python sse_agent_client.py "What is 742 Evergreen Terrace worth?"

# With both servers (pricing + inventory):
python sse_agent_client.py --both "What's the seller's minimum price?"
```
...
|---|---------|----------|----------------|
| 1 | `python sse_agent_client.py "What is 742 Evergreen Terrace worth?"` | Calls `get_market_price` → $462K | Basic SSE tool loop |
| 2 | `python sse_agent_client.py "Calculate a discount for a $485K property"` | Calls `calculate_discount` → offer ranges | LLM picks the right tool |
| 3 | `python sse_agent_client.py --both "What's the seller's minimum?"` | Calls `get_minimum_acceptable_price` → $445K | Multi-server discovery |
| 4 | `python sse_agent_client.py --both "Should I offer below asking?"` | Calls multiple tools, synthesizes answer | Multi-tool reasoning |
